...
const SEARCH_SEEDS: Array<{ query: string; category: string }> = [
...
interface LocalizedString { "@value": string }
...
interface SearchEntity {
  ...
  inclusion?: Array<{ label: LocalizedString }>;
  exclusion?: Array<{ label: LocalizedString }>;
}

interface SearchResponse { destinationEntities?: SearchEntity[] }
...
async function search(query: string, flex: boolean): Promise<SearchEntity[]> {
...
function extractTitle(title: unknown): string | null {
  ...
  let raw: string;
  if (typeof title === "string") raw = title;
  else if (typeof title === "object" && "@value" in (title as object))
    raw = (title as LocalizedString)["@value"] ?? "";
  else return null;
...
function joinLabels(
  items: Array<{ label: LocalizedString }> | undefined
): string | null {
  return items?.length
...
async function syncICD11(): Promise<void> {
  if (!CLIENT_ID || !CLIENT_SECRET) {
    console.error(
      "Set ICD11_CLIENT_ID and ICD11_CLIENT_SECRET in .env\n" +
  ...
  for (const row of existing) {
    ...
    try {
      ...
      const match =
        entities.find((e) => e.theCode?.toUpperCase() === row.code.toUpperCase()) ??
  ...
  for (const { query, category } of SEARCH_SEEDS) {
    ...
    try {
      ...
      for (const e of coded) {
        ...
        if (result.changes > 0) { added++; newCodes++; }
      }
