...
async function fetchLabel(drugName: string): Promise<OpenFDAResult | null> {
  // OpenFDA indexes names in uppercase; substance_name catches e.g. "AMOXICILLIN TRIHYDRATE"
  const searchTerm =
    `openfda.generic_name:"${drugName}" OR openfda.brand_name:"${drugName}" OR openfda.substance_name:"${drugName}"`;
...
function cap(arr: string[] | undefined, maxLen = 1800): string | null {
...
async function syncDrugs(): Promise<void> {
  initSchema();
  const db = getDb();

  const drugs = db
    .prepare("SELECT id, name, generic_name FROM drugs ORDER BY name")
    .all() as { id: number; name: string; generic_name: string | null }[];
