...
export function hasApiKeys() {
    const db = getDb();
    return db.prepare("SELECT COUNT(*) as count FROM api_keys").get().count > 0;
}
...
export function getWaitlist() {
    const db = getDb();
    return db.prepare("SELECT id, email, created_at FROM waitlist ORDER BY created_at DESC").all();
}
