...
function symptomMatches(queriedSymptom, diagnosisSymptom) {
    return diagnosisSymptom.includes(queriedSymptom) || queriedSymptom.includes(diagnosisSymptom);
}
export function getDifferentialDiagnosis(input) {
    const db = getDb();
    ...
    for (const qs of normalisedSymptoms) {
        const df = allDiagnoses.filter((dx) => dx.symptoms.split(",").map((s) => s.toLowerCase().trim()).some((ds) => symptomMatches(qs, ds))).length;
    ...
    const ranked = allDiagnoses
        ...
        const matched = normalisedSymptoms.filter((qs) => dxSymptoms.some((ds) => symptomMatches(qs, ds)));
...
//# sourceMappingURL=get_differential_diagnosis.js.map